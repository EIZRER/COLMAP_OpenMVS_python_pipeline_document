...
\begin{tikzpicture}[
    ...
    module/.style={rectangle, rounded corners=3pt, minimum width=3.0cm, minimum height=0.75cm,
                   draw=black!70, fill=blue!15, align=center, font=\small\bfseries},
    submod/.style={rectangle, minimum width=2.6cm, minimum height=0.6cm,
                   draw=black!50, fill=blue!5, align=center, font=\footnotesize},
    external/.style={rectangle, rounded corners=3pt, minimum width=2.8cm, minimum height=0.75cm,
                    draw=black!70, fill=green!15, align=center, font=\small\bfseries},
    config/.style={rectangle, rounded corners=3pt, minimum width=2.8cm, minimum height=0.75cm,
                   draw=black!70, fill=orange!15, align=center, font=\small\bfseries},
    arrow/.style={thick,->,>=stealth,draw=black!60}
]

% GUI layer
\node (gui) [module] {gui/};
\node (mw) [submod, below=0.15cm of gui] {main\_window.py};
\node (wt) [submod, below=0.1cm of mw] {worker\_thread.py};

% Pipeline layer
\node (pip) [module, right=1.6cm of gui] {pipeline/};
\node (pl) [submod, below=0.15cm of pip] {pipeline.py};
\node (cr) [submod, below=0.1cm of pl] {colmap\_runner.py};
\node (or) [submod, below=0.1cm of cr] {openmvs\_runner.py};

% Core layer
\node (core) [module, below=1.4cm of gui] {core/};
\node (co1) [submod, below=0.15cm of core] {paths.py};
\node (co2) [submod, below=0.1cm of co1] {exceptions.py};

% Utils layer
\node (utils) [module, below=1.4cm of pip] {utils/};
\node (ut1) [submod, below=0.15cm of utils] {command\_runner.py};

% Config
\node (cfg) [config, below=0.9cm of co2] {config/config.yaml};

% External tools
\node (colmap) [external, right=1.6cm of pip] {COLMAP};
\node (omvs)   [external, below=0.7cm of colmap] {OpenMVS};

% Arrows
\draw [arrow] (gui) -- (pip);
\draw [arrow] (wt.east) -- ++(0.4,0) |- (pl.west);
\draw [arrow] (cr.east) -- (colmap.west);
\draw [arrow] (or.east) -- (omvs.west);
\draw [arrow] (pip) -- (utils);
\draw [arrow] (pip.south) -- ++(0,-0.3) -| (core.north);
\draw [arrow] (cfg.north) -- (core.south);
\draw [arrow] (cfg.east) -- ++(0.3,0) |- (pip.south);
